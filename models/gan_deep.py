...
def midBlock(channel_input ,channel_mid, channel_output):
	block = nn.Sequential(
			nn.ConvTranspose2d(channel_input, channel_mid, 4, 2, 1, bias=False),
			nn.BatchNorm2d(channel_mid),
			nn.ReLU(True),
			nn.Conv2d(channel_mid, channel_output, kernel_size=3, stride=1,padding=1, bias=False),
			nn.BatchNorm2d(channel_output),
			nn.ReLU(True)
		)
	return block
...
class generator(nn.Module):
	def __init__(self):
		...
		self.block1 = midBlock(self.ngf * 8,self.ngf * 4,self.ngf * 4)
		self.residual2 = ResBlock(self.ngf * 4, self.ngf , self.ngf * 4)
		self.block2 = midBlock(self.ngf * 4, self.ngf * 4, self.ngf * 2)
		self.residual3 = ResBlock(self.ngf * 2, self.ngf, self.ngf * 2)
		self.block3 = midBlock(self.ngf * 2, self.ngf * 2, self.ngf)
		self.residual4 = ResBlock(self.ngf, self.ngf // 2, self.ngf)
